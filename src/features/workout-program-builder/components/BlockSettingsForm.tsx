...
export function BlockSettingsForm({
  ...
  if (!block) {
    return <p className="wpb-empty">설정할 블록을 선택하세요.</p>;
  }
  ...
  if (block.type === 'video') {
    ...
    return (
      <form className="wpb-form" onSubmit={(e) => e.preventDefault()}>
        <fieldset className="wpb-field">
          <legend>재생 방식</legend>
          <section className="wpb-radio-group">
        ...
        <fieldset className="wpb-field">
          <legend>음성 안내</legend>
          <section className="wpb-checkbox-grid">
            {(
              [
                ['ready', 'Ready'],
                ['go', 'Go'],
                ['stop', 'Stop'],
                ['lastTenCount', '마지막 10초 카운트'],
              ] as const
            ).map(([key, label]) => (
          ...
          <input
            ...
            onChange={(e) =>
              onUpdateBlock(block.id, (b) =>
                b.type === 'rest'
                  ? { ...b, durationSec: Number(e.target.value), title: `휴식 ${e.target.value}초` }
                  : b,
              )
            }
          />
    ...
    <form className="wpb-form" onSubmit={(e) => e.preventDefault()}>
      <label className="wpb-field">
        ...
        <input
          ...
          onChange={(e) =>
            onUpdateBlock(block.id, (b) =>
              b.type === 'voice' ? { ...b, cueText: e.target.value, title: e.target.value } : b,
            )
          }
        />
      </label>
